  ...
  if (resolution.definitions.length > 0) {
    ...
    parts.push('DEFINED ' + first.path + ':' + first.line + ' (' + kinds + ')'
      + (resolution.definitions.length > 1
        ...
        : ''))
    if (implCount > 0) parts.push(implCount + ' impl(s) of this trait')
    if (resolution.selfImpls.length > 0) parts.push('this type has ' + resolution.selfImpls.length + ' impl(s)')
  ...
  function renderReport(value) {
    ...
    for (const symbol of value.symbols) {
      ...
      for (const impl of symbol.impls) {
        lines.push('impl ' + impl.forType + '  ' + impl.path + ':' + impl.line)
      }
      // The impl SITE rows above already carry each `forType` name, so a summary
      // list is printed only when it adds names the rows did not show. The count
      // is printed regardless: a count that disagrees with the rows is exactly
      // the failure mode that sends an agent back to grep.
      if (symbol.implTargetsTotal > symbol.impls.length) {
        lines.push('implements this trait (' + symbol.implTargetsTotal + '): '
          + symbol.implTargets.join(', ')
          + (symbol.implTargets.length < symbol.implTargetsTotal
            ? ' … +' + (symbol.implTargetsTotal - symbol.implTargets.length) + ' more' : ''))
      } else if (symbol.implTargetsTotal > 0 && symbol.impls.length === 0) {
        lines.push('implements this trait (' + symbol.implTargetsTotal + '): '
          + symbol.implTargets.join(', '))
      }
      ...
      if (symbol.implForTargetsTotal > symbol.implsFor.length
        || (symbol.implForTargetsTotal > 0 && symbol.implsFor.length === 0)) {
        lines.push('this symbol implements (' + symbol.implForTargetsTotal + '): '
