...
/**
 * INDEX FRESHNESS — the one place the report's discipline used to break.
 *
 * `indexTtlMs` (10 minutes) plus `builtAtMs` on the pooled index meant that for
 * ten minutes after any edit a call was answered from a build that could predate
 * the working tree — including an `absent` verdict carrying `coverage: complete`.
 * Elsewhere this file declares what it did not search and hides nothing else; it
 * simply never said the answer could be older than the code. A stale `absent`
 * presented as final is the same class of defect as v1's "ABSENT … in 0 indexed
 * file(s)".
 *
 * CHOSEN, in two steps. First: disclose the age and do NOT downgrade the verdict
 * — the steady state is an agent asking several questions without editing
 * anything, and making every warm negative `inconclusive` would re-incite the
 * re-checking loop this plugin exists to stop (the A/B's 41 byte-identical
 * repeats). Second, and now the primary mechanism: REVALIDATE. Disclosure alone
 * still left the footgun, because the agent most likely to search next is the one
 * that just edited a file, and it has no reason to know the answer came from a
 * cache. A pooled build is therefore re-checked against the working tree before
 * it answers (see `indexIsFresh`), and rebuilt when anything moved; `reused` now
 * always means "revalidated unless `validated` is false".
 *
 * This note is now emitted ONLY when revalidation was impossible — a backend
 * whose `stat` reports no `version` token. There the answer really may predate an
 * edit, so the age disclosure and this warning still apply verbatim.
 */
const INDEX_STALENESS_NOTE = 'index staleness: a negative below is final for that build,'
  + ' not for the tree as it is now'
...
/**
 * How many near-miss candidates an `absent` verdict offers. Capped like every
 * other list, and labelled like every other capped list: the count beside it is
 * how many candidates qualified, never a limit on what was searched.
 */
...
function validateConfig(config) {
  ...
  if (config.provideSearchTools !== undefined && typeof config.provideSearchTools !== 'boolean') {
    issues.push({ message: 'provideSearchTools must be a boolean', path: ['provideSearchTools'] })
  }
  ...
  const value = {
    ...
    provideSearchTools: config.provideSearchTools === true,
  }
  for (const key of Object.keys(DEFAULT_CAPS)) {
...
/**
 * Human-scale age for the index-staleness disclosure: `0ms`, `12.3s`, `4m30s`.
 * Deliberately coarse — the caller needs to know whether the answer can predate
 * its own edits, not the exact instant.
 */
...
/**
 * How the index behind this answer came to be, in one clause, for both the
 * find_symbol header and the replacement grep/glob scope block.
 *
 * Three states, and they are distinguishable on purpose: a cold build, a reused
 * build that was REVALIDATED against the working tree before it answered, and a
 * reused build that could not be revalidated because the backend hands out no
 * freshness token — the last of which keeps the older age disclosure, because
 * there the answer really may predate an edit.
 */
function indexStateOf(index) {
  if (index.staleRebuilt === true) {
    return 'rebuilt — the pooled build was stale, the tree had changed since it was built'
  }
  if (index.reused === false) return 'built in ' + index.builtMs + 'ms'
  if (index.validated === false) return 'reused — built ' + formatAge(index.ageMs) + ' ago'
  return 'reused — built ' + formatAge(index.ageMs) + ' ago, revalidated against the tree just now'
}
...
// ---------------------------------------------------------------------------
// Subtoken normalisation — the naming-CONVENTION half of name resolution.
//
// `nameVariants` above handles Rust *syntax* (it strips `&`/`dyn`/generics and
// takes the last path segment) and `definitionsByLower` adds a case-insensitive
// tier. Neither handles *convention*: `get_user_by_id` and `getUserById` are
// different keys. Inside a pure Rust tree that costs almost nothing — Rust is
// snake_case end to end — but it costs a great deal at every boundary: serde
// `rename_all`, JSON keys, `invoke("get_user_by_id")` string literals, generated
// bindings, TS↔Rust FFI, CLI flags. Those boundaries are exactly where a model
// starts brute-forcing spellings, which is the reported problem.
//
// The split is deterministic and is applied IDENTICALLY to the index and to the
// query, so the two can only agree when the tokens agree.
// ---------------------------------------------------------------------------

/** Whether a subtoken boundary falls immediately before `chunk[index]`. */
function isSubtokenBoundary(chunk, index) {
  const before = chunk[index - 1]
  const after = chunk[index]
  const beforeDigit = before >= '0' && before <= '9'
  const afterDigit = after >= '0' && after <= '9'
  // alpha↔digit: `alpha2Beta`, `utf8`.
  if (beforeDigit !== afterDigit) return true
  const afterUpper = after >= 'A' && after <= 'Z'
  if (!afterUpper) return false
  // lower|Upper: `getUser`.
  if (!(before >= 'A' && before <= 'Z')) return true
  // UPPER|Upper: the tail of an acronym belongs to the next token, so
  // `HTTPServer` is `HTTP`+`Server` and `HttpServer` normalises the same way.
  const next = index + 1 < chunk.length ? chunk[index + 1] : ''
  return next >= 'a' && next <= 'z'
}

/**
 * Deterministic identifier splitter: camelCase boundaries, `snake_case`,
 * `kebab-case`, `SCREAMING_SNAKE`, alpha/digit transitions, case-folded, empty
 * tokens dropped.
 * @returns an array of lowercase tokens, in order.
 */
function splitSubtokens(raw) {
  const tokens = []
  // Anything that is not a letter or digit separates, so `::`, `<`, `>`, `,`,
  // `&`, `'`, `-`, `_`, `$` and whitespace all decompose identically and
  // `a::b`, `Foo<T>`, `&Bar`, `a-b` and `a_b` agree.
  for (const chunk of String(raw).split(/[^A-Za-z0-9]+/)) {
    if (chunk === '') continue
    let start = 0
    for (let index = 1; index <= chunk.length; index += 1) {
      if (index !== chunk.length && !isSubtokenBoundary(chunk, index)) continue
      tokens.push(chunk.slice(start, index).toLowerCase())
      start = index
    }
  }
  return tokens
...
/**
 * The subtoken key for a name: its tokens joined by a character that cannot
 * occur inside a token, so `a_b` (`a|b`) can never collide with a single token
 * spelled `a|b`. An identifier with no alphanumerics keys to `''` and is never
 * indexed under this tier.
 */
function subtokenKey(raw) {
  return splitSubtokens(raw).join('|')
}
...
function resolveSymbol(index, requested) {
  ...
  // LAST tier, and only when every stricter tier missed. Resolves DEFINITIONS
  // only — `implsByTrait`/`implsByFor` are deliberately NOT subtoken-keyed:
  // widening the impl sets would inflate `implTargets`, and `expectAnswer`
  // compares against exactly those, so a looser tier there could change the
  // answer to "does X implement Y" without any evidence that it does. The cost
  // is that a subtoken-resolved name reports no impls even when the convention-
  // equivalent spelling has some; that under-report is disclosed by `matchedOn`
  // and is the safe direction.
  if (definitions.length === 0) {
    for (const variant of variants) {
      const key = subtokenKey(variant)
      if (key === '') continue
      const hit = index.definitionsBySubtoken.get(key)
      if (hit !== undefined && hit.length > 0) {
        definitions = hit
        matchedName = hit[0].name
        matchedOn = 'subtoken-normalized'
        break
      }
    }
  }
...
function verdictOf(resolution, index) {
  ...
  if (resolution.definitions.length > 0) {
    ...
    } else if (resolution.matchedOn === 'subtoken-normalized') {
      // The loosest tier, so it says exactly how loose it was: no stricter form
      // of the queried spelling exists here, and what resolved is a DIFFERENT
      // spelling of the same identifier, not the name that was asked for.
      parts.push('no definition of "' + resolution.requested + '" as written; resolved by'
        + ' naming-convention normalisation to "' + name + '" (different spelling, same'
        + ' identifier tokens — no exact, path-segment or case-insensitive form matched)')
    }
...
/**
 * Path-only walk pool for the replacement `glob`. The index deliberately holds
 * only allow-listed SOURCE files, so serving `glob` from it would silently stop
 * finding `Cargo.toml`, `Makefile` or a fixture — a capability regression
 * against the built-in. This walk therefore lists every file and parses none.
 */
const WALK_POOL = new Map()

/**
 * Every file under a root, pruned by the same policy as the index, with no
 * extension filter and no file reads.
 * @returns `{ paths, prunedNames, prunedDirCount, prunedCapped, truncated }`.
 */
async function walkAllFiles(ctx, rootTarget, rootPath, includeExcluded, signal, caps, config) {
  const key = JSON.stringify([rootPath, config.excludeDirs.slice().sort(),
    includeExcluded === true, caps.maxFiles])
  const cached = WALK_POOL.get(key)
  let staleRebuilt = false
  if (cached !== undefined && Date.now() - cached.builtAtMs <= caps.indexTtlMs) {
    if (cached.validatable === false
      || await indexIsFresh(ctx, cached, signal)) {
      return { ...cached, reused: true, validated: cached.validatable !== false, staleRebuilt: false }
    }
    // Same defect as the symbol index, same fix: a cached listing that still
    // says "complete — every file under this root was listed" after a file was
    // created is a confident lie about scope.
    WALK_POOL.delete(key)
    staleRebuilt = true
  }
  const paths = []
  const dirStamps = []
  ...
  let prunedDirCount = 0
  let prunedCapped = false
  let truncated = false
  const queue = [rootTarget]
  const relativeOf = new Map([[rootTarget.targetKey, '']])
  while (queue.length > 0) {
    if (paths.length >= caps.maxFiles) { truncated = true; break }
    if (signal !== undefined && signal.aborted) throw new Error('glob: aborted while walking')
    ...
    if (info.version === undefined) validatable = false
    if (info.type === 'directory') {
      dirStamps.push({ relative, version: String(info.version) })
      let entries
      try {
      ...
      for (const entry of entries) {
        ...
        if (entry.type === 'directory') {
          if (includeExcluded !== true && config.excludeDirs.includes(entry.name)) {
            prunedDirCount += 1
            if (!prunedSeen.has(entry.name)) {
              prunedSeen.add(entry.name)
              if (prunedNames.length < MAX_REPORTED_NAMES) prunedNames.push(entry.name)
              else prunedCapped = true
            }
            continue
          }
        ...
        // `listDir` reports each child's freshness token, so a file costs no
        // extra stat to stamp.
        if (entry.version === undefined) validatable = false
        fileStamps.push({
    ...
    if (info.type === 'file') {
      ...
      paths.push(relative === '' ? baseNameOf(rootPath) : relative)
    }
  }
  paths.sort()
  const record = {
    paths, rootPath, dirStamps, fileStamps, validatable, builtAtMs: Date.now(),
    reused: false, validated: true, staleRebuilt,
    prunedNames, prunedDirCount, prunedCapped, truncated,
  }
  WALK_POOL.delete(key)
  WALK_POOL.set(key, record)
  while (WALK_POOL.size > MAX_POOL_ENTRIES) {
    const oldest = WALK_POOL.keys().next()
    if (oldest.done === true) break
    WALK_POOL.delete(oldest.value)
  }
  return record
}
...
// ---------------------------------------------------------------------------
// Near-miss candidates for a name that resolved to nothing.
//
// The tool already ranks the nearest names for a BAD PATH (see
// `rankCandidates`): a missing directory yields the closest entries inside its
// nearest existing ancestor, never a bare "cannot resolve". It did not do this
// for a bad SYMBOL — an unresolvable name produced a bare `absent` and nothing
// else, which is precisely what incites an agent to start guessing spellings.
// Name enumeration is the waste this plugin exists to eliminate (the incident
// was 394 greps and 41 byte-identical repeats), so decision #4 applies one level
// up: a negative names the closest things it SAW.
//
// The candidates come from the index alone — no extra file read, no new walk, no
// new pass — and are ADVISORY: they are ranked text, and no list here can turn
// an `absent` into anything else.
// ---------------------------------------------------------------------------

/**
 * The band of edit distance treated as "a plausible misspelling", by query
 * length. Short names need a tighter band or `id` collects half the tree; long
 * ones get a wider one because one transposition in a 20-character identifier is
 * still a plausible typo.
 */
function nearMissThreshold(name) {
...
/**
 * Case-folded Levenshtein distance, abandoned as soon as every alignment in a
 * row already exceeds `limit` — the caller only ever wants to know "within N".
 * @returns a distance, or `limit + 1` when the true distance exceeds `limit`.
 */
function boundedEditDistance(left, right, limit) {
  ...
  for (let row = 1; row <= left.length; row += 1) {
    ...
    for (let column = 1; column <= right.length; column += 1) {
      const substitute = previous[column - 1] + (left[row - 1] === right[column - 1] ? 0 : 1)
      const drop = previous[column] + 1
      const insert = current[column - 1] + 1
      let value = substitute < drop ? substitute : drop
      if (insert < value) value = insert
      current[column] = value
      if (value < best) best = value
    }
    // Every alignment in this row is already out of band, and a distance can
    // only grow from here.
...
/**
 * Every name the index saw, built once per pooled index and reused after that.
 * Definitions AND impl names, because a trait defined outside the tree is still
 * a name the index holds and a plausible target for a mistyped query. Built from
 * the stored rows, so it is bounded by the same caps as the index itself.
 */
function indexedNames(index) {
  if (index.nameCorpus === undefined) {
    const seen = new Set()
    for (const definition of index.definitions) seen.add(definition.name)
    for (const impl of index.impls) {
      if (impl.trait !== '') seen.add(impl.trait)
      seen.add(impl.forType)
    }
    index.nameCorpus = [...seen]
  }
  return index.nameCorpus
}

/**
 * The closest indexed names to a name that resolved to nothing, ranked by
 * bounded case-folded edit distance and capped at {@link NEAR_MISS_CAP}.
 * @returns `{ names, total, distance }` where `total` counts every candidate
 *   inside the band, so the rendered list can say how much it is not showing.
 */
function nearMissCandidates(index, requested, cap) {
  const needle = requested.toLowerCase()
  const limit = nearMissThreshold(needle)
  const scored = []
  for (const candidate of indexedNames(index)) {
    const lowered = candidate.toLowerCase()
    // A candidate that IS the query would have been resolved by the
    // case-insensitive tier, so it cannot occur here — but presenting it as a
    // "near miss" would be nonsense, so it is excluded rather than assumed away.
    if (lowered === needle) continue
    if (Math.abs(lowered.length - needle.length) > limit) continue
    const distance = boundedEditDistance(needle, lowered, limit)
  ...
  return {
    names: scored.slice(0, cap).map(entry => entry.candidate),
    total: scored.length,
    distance: limit,
  }
}
...
// ---------------------------------------------------------------------------
// Freshness revalidation of a pooled index.
//
// The pool serves a build for indexTtlMs, so without a check an edit made a
// second ago is invisible and an `absent` verdict is stated confidently about a
// tree that no longer exists. `refresh: true` existed, but relying on the caller
// to remember it is a footgun: the agent that just edited a file is exactly the
// agent most likely to search next, and it has no reason to know the answer came
// from a cache.
//
// `ctx.fs.stat` hands out a `version` freshness token, and the local backend
// composes it from dev:ino:size:mtimeNs:ctimeNs, so comparing it is exact rather
// than a heuristic. Directories are stamped as well as files: a directory's
// version changes when an entry is added, removed or renamed, which is the case
// a per-file mtime check cannot see — an agent that creates a new file and asks
// whether its symbol is defined is the same footgun one step earlier.
//
// The check is bounded by the index itself (one stat per indexed file plus one
// per directory walked), it short-circuits on the first mismatch, and it runs
// only when a pooled build is about to answer — a cold build needs no check
// because it just read the tree.
// ---------------------------------------------------------------------------

/** The absolute path a stamp refers to, reconstructed from the index root. */
function stampedPath(index, relative) {
  return relative === '' ? index.rootPath : joinPath(index.rootPath, relative)
}

/**
 * Revalidate a pooled index against the working tree.
 * @returns true when every stamped file and directory still reports the version
 *   it had when the index was built. Any absence, type change, version change or
 *   I/O error is a "no": an index that cannot be PROVEN current must not answer.
 */
async function indexIsFresh(ctx, index, signal) {
  for (const stamp of index.dirStamps) {
    if (stamp.version === undefined) return false
    let info
    try {
      const target = await ctx.fs.resolve(stampedPath(index, stamp.relative), { signal })
      info = await ctx.fs.stat(target, signal)
    } catch (error) {
      return false
    }
    if (info === undefined || info.type !== 'directory') return false
    if (String(info.version) !== stamp.version) return false
  }
  for (const stamp of index.fileStamps) {
    if (stamp.version === undefined) return false
    let info
    try {
      const target = await ctx.fs.resolve(stampedPath(index, stamp.relative), { signal })
    ...
    if (String(info.version) !== stamp.version) return false
    // `version` already folds in the size on the local backend; comparing it too
    // costs nothing and catches a backend whose token is coarser.
    if (stamp.size !== undefined && typeof info.size === 'number' && info.size !== stamp.size) return false
  }
  return true
}
...
function buildTool(ctx, caps, config) {
   ...
   */
  async function resolveRoot(ctx, rawPath, cwd, signal, label) {
    ...
    throw new Error(await describeMissingRoot(ctx, requested, cwd, lastError, signal, label))
  }

  /** Build the actionable text for a root that could not be resolved. */
  async function describeMissingRoot(ctx, requested, cwd, lastError, signal, label) {
    const lines = [label + ': NOTHING WAS SEARCHED — ' + JSON.stringify(requested)
  ...
  async function buildIndex(ctx, rootTarget, rootKind, rootPath, include, includeExcluded, signal, effectiveConfig) {
    ...
    if (cached !== undefined && Date.now() - cached.builtAtMs <= caps.indexTtlMs) {
      if (cached.validatable === false) {
        // This backend hands out no freshness token, so the build cannot be
        // revalidated. It answers, and the report discloses the build's age
        // instead of claiming it is current — the pre-revalidation contract.
        return { index: cached, reused: true, validated: false, staleRebuilt: false }
      }
      if (await indexIsFresh(ctx, cached, signal)) {
        return { index: cached, reused: true, validated: true, staleRebuilt: false }
      }
      // The tree changed under the pooled build. Drop it and rebuild below, so
      // the caller's answer describes the tree as it is now, not as it was.
      INDEX_POOL.delete(key)
      staleRebuilt = true
    }
    const running = INDEX_INFLIGHT.get(key)
    if (running !== undefined) {
      return { index: await running, reused: true, validated: true, staleRebuilt }
    }
    const build = (async () => {
      ...
      const index = {
        ...
        prunedDirCount: 0,
        prunedCapped: false,
        // Freshness stamps, so a REUSED index can be revalidated against the
        // working tree instead of answering from a build that may predate an
        // edit. One per directory walked (a changed directory version means an
        // entry was added, removed or renamed — the case a per-file mtime check
        // cannot see) and one per indexed file. A file the index SKIPPED
        // (oversize, binary, unreadable) is deliberately not stamped: it
        // contributed nothing, and its presence already makes the coverage
        // `partial`, so a verdict there is `inconclusive` rather than a final
        // negative that a stamp would have to protect.
        dirStamps: [],
        fileStamps: [],
        // Cleared by the first stat that reports no `version`: a backend that
        // cannot hand out freshness tokens cannot be revalidated, and the report
        // falls back to disclosing the build's age instead of claiming currency.
        validatable: true,
      }
      const prunedSeen = new Set()
      const queue = [rootTarget]
      ...
      while (queue.length > 0) {
        ...
        if (info.version === undefined) index.validatable = false
        if (info.type === 'directory') {
          index.dirStamps.push({ relative, version: String(info.version) })
          let entries
          try {
            entries = await ctx.fs.listDir(target, signal)
          } catch (error) {
      ...
      // The last definition tier: one key per naming convention, so
      // `get_user_by_id`, `getUserById`, `get-user-by-id`, `user_profile` and
      // `userProfile` reach the same entry. Definitions only — see the note in
      // `resolveSymbol` for why the impl maps are deliberately left as they are.
      index.definitionsBySubtoken = new Map()
      for (const definition of index.definitions) {
        const key = subtokenKey(definition.name)
        if (key === '') continue
        const bucket = index.definitionsBySubtoken.get(key)
        if (bucket === undefined) index.definitionsBySubtoken.set(key, [definition])
    ...
    try {
      return { index: await build, reused: false, validated: true, staleRebuilt }
    } finally {
  ...
  function renderReport(value) {
    ...
    lines.push('index ' + value.index.id + ': ' + value.index.files + ' files, '
      ...
      + indexStateOf(value.index) + ')')
    if (value.index.reused && value.index.validated === false) lines.push(INDEX_STALENESS_NOTE)
    ...
    for (const symbol of value.symbols) {
      ...
      // Advisory only, and it says so: the verdict above stands whatever this
      // list contains. Ranked and capped in the same idiom as every other list
      // here — the first number is what is shown, the second is the complete
      // count, and the label makes clear the list is what is capped.
      if (symbol.nearMiss.length > 0) {
        lines.push('near-miss ' + symbol.nearMiss.length + ' of ' + symbol.nearMissTotal
          + ' name(s) within edit distance ' + symbol.nearMissDistance + ' of "' + symbol.name
          + '" (candidates from the index, advisory — the verdict above is unchanged): '
          + symbol.nearMiss.join(', ')
          + (symbol.nearMissTotal > symbol.nearMiss.length
            ? ' … +' + (symbol.nearMissTotal - symbol.nearMiss.length) + ' more' : ''))
      }
    }
  ...
  const tool = {
    ...
    async execute(args, exec) {
      ...
      const root = await resolveRoot(ctx, args.path, cwd, signal, config.toolName)
      ...
      const { index, reused, validated, staleRebuilt } = await buildIndex(
      ...
      const symbols = resolutions.map(entry => {
        ...
        // Only an `absent` verdict gets candidates. An `impls-only` or
        // `mentions-only` answer already names the sites that resolved it; a
        // list of alternative spellings beside those would be noise.
        const nearMiss = resolved.status === 'absent'
          ? nearMissCandidates(index, entry.requested, NEAR_MISS_CAP)
          : { names: [], total: 0, distance: 0 }
        return {
          ...
          verdict,
          nearMiss: nearMiss.names,
          nearMissTotal: nearMiss.total,
          nearMissDistance: nearMiss.distance,
      ...
      return {
        ...
        index: {
          ...
          reused,
          ageMs: reused ? Math.max(0, Date.now() - index.builtAtMs) : 0,
          validated: validated !== false,
          staleRebuilt: staleRebuilt === true,
      ...
      schema: {
        ...
        properties: {
          ...
          index: {
            type: 'object',
            additionalProperties: false,
            required: ['id', 'files', 'definitions', 'impls', 'builtMs', 'reused', 'ageMs',
              'validated', 'staleRebuilt', 'skipped',
          ...
          symbols: {
            ...
            items: {
              ...
              required: ['name', 'matchedName', 'matchedOn', 'status', 'verdict', 'definitions',
                ...
                'mentions', 'coverage', 'nearMiss', 'nearMissTotal', 'nearMissDistance'],
              properties: {
                name: { type: 'string' },
                matchedName: { type: 'string' },
                matchedOn: {
                  type: 'string',
                  enum: ['exact', 'path-segment-or-generic', 'case-insensitive', 'subtoken-normalized'],
                },
                status: {
                  type: 'string',
                ...
                nearMissTotal: { type: 'integer' },
                nearMissDistance: { type: 'integer' },
  ...
  /**
   * Resolve an `include` override shared by the replacement search tools.
   * @param rawInclude - the call's `include`, or undefined for the mount default.
   * @param label - tool name used in the error, so the message names the caller.
   */
  function resolveInclude(rawInclude, label) {
    if (rawInclude === undefined) return config.include
    if (!Array.isArray(rawInclude) || rawInclude.length === 0
      || rawInclude.some(entry => typeof entry !== 'string' || !entry.startsWith('.'))) {
      throw new Error(label + ': `include` must be a non-empty array of extensions starting with "."')
    }
    return rawInclude.map(entry => entry.toLowerCase())
  }

  /** The shared scope-evidence block, rendered once per search report. */
  ...
  /** The scope-evidence payload shared by the replacement search tools. */
  function scopeValue(index, reused, validated, staleRebuilt) {
    const searchedCounts = new Map()
    for (const relativePath of index.filePaths) {
      const extension = extensionOf(relativePath)
      const key2 = extension === '' ? '(none)' : extension
    ...
    const searched = [...searchedCounts.entries()]
      ...
      .map(([extension, files]) => ({ extension, files }))
    const notSearchedAll = [...index.census.entries()]
      .filter(([extension]) => !config.include.includes(extension))
      .sort((left, right) => right[1] - left[1])
    const notSearched = notSearchedAll.slice(0, MAX_REPORTED_NAMES)
      .map(([extension, files]) => ({ extension, files }))
    return {
  ...
  /** The index shape both replacement tools declare in their output schema. */
  const INDEX_SCHEMA = {
    type: 'object',
    additionalProperties: false,
    required: ['id', 'files', 'builtMs', 'reused', 'ageMs', 'validated', 'staleRebuilt', 'searched',
      'notSearched', 'notSearchedMore', 'prunedNames', 'prunedDirs', 'prunedCapped', 'coverage'],
    properties: {
      id: { type: 'string' },
      files: { type: 'integer' },
      builtMs: { type: 'number' },
      ...
      searched: {
        ...
        items: {
          ...
          properties: { extension: { type: 'string' }, files: { type: 'integer' } },
        },
      },
      notSearched: {
        type: 'array',
  ...
  /**
   * Compile a shell-style glob to a RegExp. `**` spans separators, `*` and `?`
   * do not, and a pattern with no `/` matches the basename at any depth.
   */
  ...
  /**
   * The drop-in `grep`. Same call shape as the built-in, but backed by the same
   * index and — the point — carrying the same scope evidence, so a negative is
   * trustworthy and a bad path is a hard error that read nothing.
   */
  function buildGrepReplacement() {
    return {
      name: 'grep',
      description: 'Search file contents. Returns path:line:text matches with an EXACT total, and'
        + ' states exactly which files and extensions were searched, which were pruned, and whether'
        + ' any cap made the result incomplete. A path that does not exist is a hard error that read'
        + ' nothing — never an empty result.',
      parameters: {
        type: 'object',
        additionalProperties: false,
        required: ['pattern'],
        properties: {
          pattern: {
            type: 'string',
            description: 'Literal text to find. Treated as a regular expression when `regex` is true.',
          },
          regex: { type: 'boolean', description: 'Treat `pattern` as a JavaScript regular expression.' },
          path: { type: 'string', description: 'Root file or directory. Defaults to the session working directory.' },
          include: {
            type: 'array',
            items: { type: 'string' },
            description: 'Extension allow-list override, e.g. [".rs"].',
          },
          maxMatches: { type: 'number', description: 'Cap on returned match rows; the total is always exact.' },
        },
      },
      timeoutMs: caps.timeoutMs,
      isConcurrencySafe: () => true,
      async execute(args, exec) {
        ...
        const sessionCwd = exec === undefined || exec.agent === undefined
          ...
          : exec.agent.session.header.cwd
        const cwd = sessionCwd === undefined ? fallbackCwd() : sessionCwd
        if (typeof args !== 'object' || args === null) throw new Error('grep: arguments must be an object')
        if (typeof args.pattern !== 'string' || args.pattern === '') {
          throw new Error('grep: `pattern` must be a non-empty string')
        }
        const include = resolveInclude(args.include, 'grep')
        const cap = args.maxMatches === undefined
          ? caps.maxTextSites
          : Math.min(Math.floor(args.maxMatches), 2000)
        let matcher = literalMatcher
        if (args.regex === true) {
          let expression
          try {
            expression = new RegExp(args.pattern, 'g')
          } catch (error) {
        ...
        const root = await resolveRoot(ctx, args.path, cwd, signal, 'grep')
        const { index, reused, validated, staleRebuilt } = await buildIndex(
          ctx, root.target, root.kind, root.rootPath, include, false, signal, config)
        const hits = await scanText(ctx, index, [args.pattern], include, signal, cap, matcher)
        const record = hits.get(args.pattern)
        const fileSet = new Set(record.sites.map(site => site.path))
        return {
          ...
          capped: record.count > record.sites.length,
          sites: record.sites,
          index: scopeValue(index, reused, validated, staleRebuilt),
        }
      },
      output: {
        schema: {
          type: 'object',
          additionalProperties: false,
          required: ['pattern', 'regex', 'count', 'files', 'capped', 'sites', 'index'],
          properties: {
            pattern: { type: 'string' },
            regex: { type: 'boolean' },
            count: { type: 'integer' },
            ...
            sites: {
              type: 'array',
              items: {
                type: 'object', additionalProperties: false, required: ['path', 'line', 'text'],
                properties: {
                  path: { type: 'string' },
                  line: { type: 'integer' },
                  text: { type: 'string' },
        ...
        render(_args, value) {
          const lines = ['grep ' + (value.regex ? '(regex) ' : '') + JSON.stringify(value.pattern)
            + ' \u00b7 ' + value.count + ' match(es) in ' + value.files + ' file(s)']
          for (const line of scopeLines(value)) lines.push(line)
          for (const site of value.sites) lines.push(site.path + ':' + site.line + ': ' + site.text)
          if (value.capped) {
            lines.push('\u2026 showing ' + value.sites.length + ' of ' + value.count
              + ' matches (the total is exact; raise maxMatches for the rest)')
          }
          if (value.count === 0) {
            lines.push('No matches. The scope above is the complete set of files searched, so this'
              + ' negative is final for that scope.')
          }
  ...
  /** The drop-in `glob`, served from the files the index actually walked. */
  function buildGlobReplacement() {
    return {
      name: 'glob',
      description: 'Find files by path pattern (e.g. "**/*.rs", "*.toml"). Returns paths plus the'
        + ' exact directories and extensions that were excluded, so an empty result is explainable.',
      parameters: {
        type: 'object',
        additionalProperties: false,
        required: ['pattern'],
        properties: {
          pattern: { type: 'string', description: 'Shell-style pattern; `**` spans directories, `*` and `?` do not.' },
          path: { type: 'string', description: 'Root directory. Defaults to the session working directory.' },
          include: {
            type: 'array',
            items: { type: 'string' },
            description: 'Extension allow-list override, e.g. [".rs"].',
          },
          maxResults: { type: 'number', description: 'Cap on returned paths; the total is always exact.' },
        },
      },
      timeoutMs: caps.timeoutMs,
      isConcurrencySafe: () => true,
      async execute(args, exec) {
        ...
        const sessionCwd = exec === undefined || exec.agent === undefined
          ...
          : exec.agent.session.header.cwd
        const cwd = sessionCwd === undefined ? fallbackCwd() : sessionCwd
        if (typeof args !== 'object' || args === null) throw new Error('glob: arguments must be an object')
        if (typeof args.pattern !== 'string' || args.pattern === '') {
          throw new Error('glob: `pattern` must be a non-empty string')
        }
        const cap = args.maxResults === undefined
          ? 200
          : Math.min(Math.floor(args.maxResults), 5000)
        const expression = globToRegExp(args.pattern)
        const root = await resolveRoot(ctx, args.path, cwd, signal, 'glob')
        const walk = await walkAllFiles(ctx, root.target, root.rootPath,
          args.includeExcluded === true, signal, caps, config)
        const matched = walk.paths.filter(relativePath => expression.test(relativePath))
        const parts = []
        parts.push(walk.paths.length + ' file(s) walked')
        if (walk.prunedDirCount > 0) {
          parts.push(walk.prunedDirCount + ' dir(s) pruned by policy (' + walk.prunedNames.join(', ')
            + (walk.prunedCapped ? ' \u2026' : '') + ')')
        }
        if (walk.truncated) parts.push('the walk stopped at maxFiles')
        return {
          pattern: args.pattern,
          count: matched.length,
          capped: matched.length > cap,
          paths: matched.slice(0, cap),
          walk: {
            files: walk.paths.length,
            prunedNames: walk.prunedNames,
            prunedDirs: walk.prunedDirCount,
            prunedCapped: walk.prunedCapped,
            truncated: walk.truncated,
            reused: walk.reused === true,
            validated: walk.validated !== false,
            staleRebuilt: walk.staleRebuilt === true,
            coverage: walk.truncated
              ? 'INCOMPLETE \u2014 the walk stopped at maxFiles, so this listing is a lower bound'
              : (walk.prunedDirCount > 0
                ? 'complete for every path outside the pruned directories named above'
                : 'complete \u2014 every file under this root was listed'),
          },
        }
      },
      output: {
        schema: {
          type: 'object',
          additionalProperties: false,
          required: ['pattern', 'count', 'capped', 'paths', 'walk'],
          properties: {
            pattern: { type: 'string' },
            count: { type: 'integer' },
            capped: { type: 'boolean' },
            ...
            walk: {
              type: 'object',
              additionalProperties: false,
              required: ['files', 'prunedNames', 'prunedDirs', 'prunedCapped', 'truncated', 'coverage',
                'reused', 'validated', 'staleRebuilt'],
              properties: {
                files: { type: 'integer' },
                reused: { type: 'boolean' },
                validated: { type: 'boolean' },
        ...
        render(_args, value) {
          const lines = ['glob ' + JSON.stringify(value.pattern) + ' \u00b7 ' + value.count + ' file(s)']
          lines.push('walked ' + value.walk.files + ' file(s)'
            + (value.walk.prunedDirs > 0
              ? ' \u00b7 pruned: ' + value.walk.prunedNames.join(', ') + ' (' + value.walk.prunedDirs + ')'
              : ''))
          lines.push('coverage: ' + value.walk.coverage)
          for (const path of value.paths) lines.push(path)
          if (value.capped) {
            lines.push('\u2026 showing ' + value.paths.length + ' of ' + value.count
              + ' paths (the total is exact; raise maxResults for the rest)')
          }
          if (value.count === 0) {
            lines.push('No files matched. The coverage line above is the whole basis for that answer.')
          }
          return [{ type: 'text', text: lines.join('\n') }]
        },
      },
  ...
  // WHY A DROP-IN REPLACEMENT RATHER THAN A REDIRECT. `tools/pre-execute` can
  // allow, deny, or ask — it cannot substitute a result — so redirecting a grep
  // would cost one round trip per search and, worse, adds doctrine. The A/B
  // measured where doctrine leads: the only arm to lose correctness is the one
  // that was told how to work, and it lost it by second-guessing an answer that
  // was already correct. A `grep` that simply returns scoped, exact,-count
  // results needs no instruction and cannot be second-guessed into being wrong.
  //
  // Opt-in, because it must be paired with disabling the row that provides the
  // built-in `grep`: both register into the same preset layer, and a duplicate
  // name in one layer is an error rather than a shadow.
  const extras = config.provideSearchTools === true
    ? [buildGrepReplacement(), buildGlobReplacement()]
    : []
  return { tool, extras }
}
...
function apply(ctx, config) {
  ...
  try {
    ...
    const built = buildTool(ctx, caps, config)
    // Extras first so a test or a reader that takes the LAST registration still
    // gets the tool the row names.
    for (const extra of built.extras) ctx.tools.register(extra)
    ctx.tools.register(built.tool)
    report('registered')
  } catch (error) {
