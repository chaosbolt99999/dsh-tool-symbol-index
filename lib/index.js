...
/**
 * DISCLOSURE OF INDEX AGE — the one place the report's discipline used to break.
 *
 * `indexTtlMs` (10 minutes) plus `builtAtMs` on the pooled index means that for
 * ten minutes after any edit, a call is answered from a build that may predate
 * the working tree — including an `absent` verdict that carries
 * `coverage: complete`. Elsewhere this file declares what it did not search and
 * hides nothing else; it simply never said the answer could be older than the
 * code. A stale `absent` presented as final is the same class of defect as v1's
 * "ABSENT … in 0 indexed file(s)".
 *
 * CHOSEN: ACCOMPANY THE AGE, DO NOT DOWNGRADE THE VERDICT. Every reused answer
 * states the age of the build it came from, in the header, and carries this line
 * — so an `absent` reached through a reused index is always accompanied by it.
 * Downgrading every warm negative to `inconclusive` was the alternative and was
 * rejected: the steady state is an agent asking several questions without
 * editing anything, and turning those answers non-final would re-incite exactly
 * the re-checking loop this plugin exists to stop (the A/B's 41 byte-identical
 * repeats). The disclosure also does not attempt a content revision: that needs
 * a re-stat or re-read of the tree, i.e. the rebuild-triggering change this one
 * deliberately is not. `refresh: true` rebuilds on demand.
 */
...
async function walkAllFiles(ctx, rootTarget, rootPath, includeExcluded, signal, caps, config) {
  ...
  if (cached !== undefined && Date.now() - cached.builtAtMs <= caps.indexTtlMs) return cached
  const paths = []
  const prunedNames = []
  ...
  while (queue.length > 0) {
    ...
    if (info.type === 'directory') {
      ...
      for (const entry of entries) {
        ...
        paths.push(relative === '' ? entry.name : childRelative)
      }
      continue
    }
    if (info.type === 'file') paths.push(relative === '' ? baseNameOf(rootPath) : relative)
  }
  paths.sort()
  const record = { paths, builtAtMs: Date.now(), prunedNames, prunedDirCount, prunedCapped, truncated }
...
function buildTool(ctx, caps, config) {
  ...
  function renderReport(value) {
    ...
    lines.push('index ' + value.index.id + ': ' + value.index.files + ' files, '
      ...
      + (value.index.reused
        ? 'reused — built ' + formatAge(value.index.ageMs) + ' ago'
        : 'built in ' + value.index.builtMs + 'ms') + ')')
    if (value.index.reused) lines.push(INDEX_STALENESS_NOTE)
      ...
      schema: {
        ...
        properties: {
          ...
          index: {
            type: 'object',
            additionalProperties: false,
            required: ['id', 'files', 'definitions', 'impls', 'builtMs', 'reused', 'ageMs', 'skipped',
  ...
  function scopeValue(index, reused) {
    const searchedCounts = new Map()
    for (const relativePath of index.filePaths) {
      const extension = extensionOf(relativePath)
      const key2 = extension === '' ? '(none)' : extension
  ...
  const INDEX_SCHEMA = {
    type: 'object',
    additionalProperties: false,
    required: ['id', 'files', 'builtMs', 'reused', 'ageMs', 'searched', 'notSearched', 'notSearchedMore',
      'prunedNames', 'prunedDirs', 'prunedCapped', 'coverage'],
    properties: {
      id: { type: 'string' },
      files: { type: 'integer' },
      builtMs: { type: 'number' },
